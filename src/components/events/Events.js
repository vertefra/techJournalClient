  ...
  return (
    <Layout>
      <div className="AllPublicEventsContainer">
        ...
        <p className="p-AllEvents">
          Here you'll be able to explore all events created by our users and
          click on "Learn More" form more info about each event. From there,
          you'll be able to click on "Will Attend" to add that event to your
          events you'll attend.
        </p>
        {allEvents.length > 0 &&
          allEvents.map((event, idx) => {
            return <CardEvent key={idx} event={event} />;
          })}
      </div>
    </Layout>
  );
